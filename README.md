...
**This line (`8.0.0.1`):** Java **8**, **`javax.xml.ws`** / **`javax.xml.bind`**, **15** facade clients. Runtime SOAP stack is Eclipse EE4J **`com.sun.xml.ws:jaxws-rt`** **2.3.7**. Pair with **`au.gov.nehta:mhr-wsdl`** **`8.0.0.1`**. Do **not** use legacy Metro **`webservices-*`** bundles.
...
| ------------- | ------- | --------- | ------- |
| **8.0.0.1** | **8** | **`javax.*`** / EE4J **`jaxws-rt` 2.3.x** | **15** (MHR B2B) |
...
The **8.0.0.1** JAR ships **15** MHR B2B facade clients on **`javax`**. **11.0.0.1** and later lines use **Jakarta** with the same facade count.
Java 8 is no longer a supported platform; consumers of this client should upgrade to a current Java LTS version.

---

## Local development (SNAPSHOT)

This repository builds **`8.0.0.1-SNAPSHOT`**. Install unpublished **`mhr-wsdl`** **first** at the **same Maven version**, then **`verify`** here:
...
If Maven warns that a **GA** POM is missing (for example **`8.0.0.1`** before Central publish), clear stale **`au/gov/nehta/mhr-wsdl`** entries in your **local Maven repository** (folders with only **`.lastUpdated`** files) and reinstall the SNAPSHOT. **`mvn clean`** in this project does not clear the local repository cache.
...
| Repository | Role |
| ---------- | ---- |
| [mhr-wsdl-java](https://github.com/AuDigitalHealth/mhr-wsdl-java) | Generated MHR SOAP types (**8.0.0.1**, Java 8 / `javax`) |
