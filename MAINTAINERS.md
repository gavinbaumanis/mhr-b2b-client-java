...
| ------- | ---- | -------- | ------- |
| **8.0.0.1** | 8 | **`javax.xml.ws`**, **`javax.xml.bind`**, **`javax.jws`** | **15** (MHR B2B) |
...
**This checkout (`8.0.0.1-SNAPSHOT`):** Java **8**, **`javax`** facade clients, **`au.gov.nehta:mhr-wsdl`** at **`${project.version}`**, **15** facades (no **`wsimport`** in the default lifecycle). Stack and **`.github/workflows/ci.yml`** (branch **`java-8`**, JDK **8**, checkout **`mhr-wsdl-java`** **`java-8`**) below apply to **this line only**.
...
- **`.github/workflows/ci.yml`:** GitHub Actions on **`java-8`**, JDK **8**; installs **`mhr-wsdl`** **`8.0.0.1-SNAPSHOT`** then **`verify`**
...
Git/SCM settings for **`maven-release-plugin`** live in **`pom.xml`** properties (**`scm.repo.url`**, **`release.*`**). Tags default to **`{artifactId}-{version}`** (e.g. **`mhr-b2b-client-8.0.0.1`**).
...
```text
mvn -B "-Prelease" release:prepare release:perform -DreleaseVersion=8.0.0.1 -DdevelopmentVersion=8.0.0.2-SNAPSHOT -Dtag=mhr-b2b-client-8.0.0.1
```
